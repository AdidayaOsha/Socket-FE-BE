...
function App() {
  ...
  useEffect(() => {
    socket.emit(namespace, { name: user });
    socket.on(namespace, (data) => {
      alert(`${data} has entered the chat`);
    });
    socket.on(`${namespace}User`, (data) => {
      alert(`${data} has entered Room 1`);
    });
    socket.on(`${namespace}Chat`, (data) => setMessages(data));
    socket.on(`${namespace}Room`, (data) => setRoomMessage(data));
    return () => {
      socket.close();
    };
  }, [namespace]);
